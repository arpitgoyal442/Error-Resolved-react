...
const MobileDoubtPage = ({ aboutDoubt }) => {
	const [showChat, setShowChat] = useState(false),
		[viewType, setViewType] = useState(3); // 1 = screen, 2 = code, 3 = document
	...
	return (
		<>
			...
			<div className="md:hidden p-3 flex h-[90vh] flex-col">
				{/* Topic */}
				<div className="rounded-md bg-darkest p-2 text-center font-semibold">{aboutDoubt.topic}</div>
				{/* Screen */}
				<div className="mt-4 flex-1">
					<div className="border-2 rounded-md w-full h-full bg-light">
						{viewType === 3 && <Document />}
					</div>
				</div>
				{/* View Options */}
	...
	return (
		<div className="bg-light absolute top-0 left-0 w-screen h-screen pt-navbar flex flex-col">
						...
						<div
							...
							className={
								message.senderId == currentId
									? "mobileChatDiv mobileChat_receiver"
									: "mobileChatDiv mobileChat_sender"
							}
						>
			...
			<div className="border-t border-t-black p-2 flex items-center space-x-2">
				...
				<form onSubmit={sendMessage} className="bg-white rounded-full flex items-center flex-1 p-2 pl-4">
					<input
						value={message}
						onChange={(e) => {
