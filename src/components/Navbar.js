...
function Navbar() {
	...
	useEffect(() => {
		async function fetchUserProfile() {
			let fetchUrl = "";
			if (userType == 1) fetchUrl = `${URL}/student/profile/` + userId;
			else fetchUrl = `${URL}/debugger/profile/` + userId;

			let userProfile = await axios.get(fetchUrl).catch((err) => {
				return err;
			});
	...
	return (
		<div className="navbar" ref={navRef}>
			<Link to={userType == 1 ? "/student" : "/debugger"} className="navbar__homeLink">
				<h2>
			...
			<div className="navbarIcons">
				<div className="dropdown">
					<div className="w-8 h-8 relative">
						<p className="grid place-items-center w-4 h-4 text-xs text-white font-semibold rounded-full bg-highlight absolute top-0 right-0 -translate-y-1/3">
								...
								return (
									<li key={index}>
										<DropdownContent
											...
											userData={userData}
											closeDropdown={() => setNotification(false)}
										/>
									</li>
								);
							})}
						...
						<ul>
								...
								<GoogleLogout
									...
									render={(renderProps) => (
										<button onClick={renderProps.onClick} disabled={renderProps.disabled}>
											<span onClick={onLogout} className="logouticon">
												{" "}
												<LogoutIcon className="h-9 w-9 text-gray-500 logouticon" />{" "}
											</span>{" "}
											Logout{" "}
										</button>
									)}
							...
							<div>
								<span className="logouticon">
									{" "}
									<UserIcon className="h-9 w-9 text-gray-500 logouticon" />{" "}
								</span>{" "}
								Profile
							</div>
						</ul>
